        ...
        if result and self.erreurs_verif_apres and self.delai_verification > 0:
            ...
            for err in self.erreurs_verif_apres:
                ...
                if err.condition():  # Erreur détectée
                    ...
                    self.logger.warning(
                        f"Erreur post-action : {err.message or err.image}"
                    )
                    result = False
                    ...
                    if err.retry_action_originale:
                        self.logger.info(
                            "Retry de l'action après correction"
                        )
        ...
        if not result:
            ...
            for err in self.erreurs_si_echec:
                ...
                if err.condition():
                    erreur_identifiee = True
                    self.logger.warning(
                        f"Erreur si_echec : {err.message or err.image}"
                    )
                    err.execute()

                    if err.retry_action_originale:
                        self.logger.info(
                            "Retry de l'action après correction (si_echec)"
                        )
                        self.reset_condition()
                        self._deja_retry_non_identifie = False
                        result = self._run()
            ...
            if not erreur_identifiee and self.retry_si_erreur_non_identifiee:
                if not self._deja_retry_non_identifie:
                    ...
                    self.logger.warning(
                        "Erreur non identifiée - Retry automatique (1 fois)"
                    )
                    self.reset_condition()
