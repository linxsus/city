...
from utils.config import TIMERS_FILE, DATA_DIR
    ...
    def get_due_timers(self, fenetre_id=None):
        ...
        with self._lock:
            for timer in self._timers.values():
                if timer.is_due():
                    if fenetre_id is None or timer.fenetre_id == fenetre_id or timer.fenetre_id is None:
    ...
    def get_next_due_time(self, fenetre_id=None):
        ...
        with self._lock:
            for timer in self._timers.values():
                if not timer.actif:
                    continue
                if fenetre_id is not None and timer.fenetre_id != fenetre_id and timer.fenetre_id is not None:
                    continue
