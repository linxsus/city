    ...
    def clear_for(self, fenetre_id):
        ...
        with self._lock:
            self._messages = deque(
                [m for m in self._messages if not m.is_for(fenetre_id)],
...
def demander_raid(source, destination, details=None):
    ...
    return get_message_bus().send_to(
        source, destination, 
        MessageType.DEMANDE_RAID, 
        details
    )
...
def signaler_kills_termines(source, nb_kills=None):
    ...
    return get_message_bus().broadcast(
        source,
        MessageType.KILLS_TERMINES,
        {'nb_kills': nb_kills}
    )
